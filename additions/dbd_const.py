...
create table dbd$schemas (
    ...
    version varchar default(null), -- версия схемы
    fulltext_engine varchar default(null), -- название полнотекстового движка
...
create table dbd$tables (
    ...
    description varchar default(null),    -- описание
    ht_table_flags varchar default(null), -- флаги доступа (чтение/запись/поиск)
    access_level integer default(null),   -- уровень доступа
...
create index "idx.12XXTJUYZ" on dbd$indexes(table_id);
create index "idx.6G0KCWN0R" on dbd$indexes(name);
create index "idx.FQH338PQ7" on dbd$indexes(uuid);
--
-- Детали индексов
--
...
create view dbd$view_indexes as
select
  dbd$indexes.id "index_id",
  dbd$indexes.name as index_name,
  dbd$schemas.name "schema",
...
from
  dbd$index_details
  inner join dbd$indexes on dbd$index_details.index_id = dbd$indexes.id
  inner join dbd$tables on dbd$indexes.table_id = dbd$tables.id
  left join dbd$fields on dbd$index_details.field_id = dbd$fields.id
  Left Join dbd$schemas On dbd$tables.schema_id = dbd$schemas.id
order by
  dbd$tables.name, dbd$indexes.name, dbd$index_details.position;
"""
